package Dominio.Funcionario.Funcionario;
...
import Dominio.Funcionario.Funcionario.Exceptions.*;
...
public abstract class Funcionario {
    ...
    // -- Construtor com ID -- //
    public Funcionario(long id, NomeFuncionario nomeFuncionario, CPF cpf, Senha senha, NivelAcesso nivelAcesso, ListaDepartamentos listaDepartamentos) {
        alteraListaDepartamentos(listaDepartamentos);
        alteraIdUsuario(id);
        alteraNome(nomeFuncionario);
    ...
    public void alteraListaDepartamentos(ListaDepartamentos listaDepartamentos) {
        if(listaDepartamentos == null) {
            throw new DepartamentoException("A lista de departamentos de um funcionário não pode ser vazia");
        }
    ...
    public void alteraIdUsuario(long id) {
        if(id < 0) {
            throw new IdFuncionarioException("Um ID de funcionário não pode ser menor que 0");
        }
    ...
    public void adicionarDepartamento(Departamento departamento) {
        if(departamento == null) {
            throw new DepartamentoException("Um funcionário só pode possuir departamentos válidos");
        }
    ...
    public boolean igualMeuId(long idComparado) {
        return this.idFuncionario == idComparado;
    }
